...
```
  ...
  * La programación orientada a objetos es un paradigna de programación que otorga los medios para estructurar programas de tal manera que las propiedades y comportamietnos estén envueltos en objetos individuales.
...
+ Cuando necesitamos crear estructuras más complejas (por ejemmplo, un avión), podemos utilizar clases-
+ La instalncia es el objeto concreto con valores reales.
...
En cualquier momento determinado, el programa tiene acceso a tres scopes:
  ...
  Cuando se solicita un objeto, Python busca primero el nombre en el scope local, luego en el global, y por último, en el raíz. Cuando anidamos una función dentro de otra función, su scope también que `(other_local_name)`
...
def some_function():
     ...
     Some_var_in_other_scope += 1eda anidado dentro del scope de la función padre.
